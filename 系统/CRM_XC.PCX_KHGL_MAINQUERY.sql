CREATE OR REPLACE PROCEDURE CRM_XC.PCX_KHGL_MAINQUERY(O_CODE         OUT NUMBER,
                                               O_NOTE         OUT VARCHAR2,
...
BEGIN
  ...
  IF I_TOTALROWS = -1 THEN
    --计算总的行数
    PRO_CAL_TOTALROWS(I_TOTALROWS, O_NOTE, SQLS, GROUPISLAST);
    IF I_TOTALROWS = 0 THEN
      ...
      IF O_HASRECORDSET = 1 THEN
        ...
        ELSE
          VSQLS := SQLS;
        END IF;
        /*INSERT INTO CRMII.TEST_PARA
          (SJ, PROC, SQLS)
        VALUES
          (SYSDATE, 'PCX_KHGL_MAINQUERY', VSQLS);
        COMMIT;*/
  ...
  IF O_HASRECORDSET = 1 THEN
    ...
    V_SORTSQL := CASE
                 ...
                 END;
    IF I_PAGING = 1
       AND I_TOTALROWS > I_PAGELENGTH THEN
      --如果分页
      --分页请处理有关
