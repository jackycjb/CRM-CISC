...
Create Global Temporary Table temp_tables On Commit Preserve Rows as
select row_number() over(order by owner,table_name) id,owner,table_name,count(table_name) comments_c from all_col_comments 
where  comments<>' ' and table_name not like '%TMP%' and table_name not like '%TEMP%' and owner not in('SYS','SYSTEM','XDB','USR')
group by owner,table_name having(count(table_name) >10) 
